...
// and run in O(n), taking advantage that suffixes already is sorted by
...
int main(){
	...
	string s;
	cin>>s>>k;
	vector<int> text(s.size());
	for(int i=0; i<s.size(); i++)
	text[i] = s[i] - 'a';
	sa.Build(text, 26);
	sa.BuildLcpTable();
	for(int i=0; i<sa.sortedIds.size(); i++){
		int id = sa.sortedIds[i];
		int diffstrs = s.size() - id - sa.lcp[i];
		if(diffstrs < k)
		k -= diffstrs;
		else{
			int size = sa.lcp[i] + k;
			for(int j=id; j<(id+size); j++)
			cout<<s[j]; cout<<endl;
			break;
		}
	}
