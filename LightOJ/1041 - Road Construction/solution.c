...
int main(){
	int t,l,suma;
	int TotalVertex,TotalEdges,vertex1,vertex2,weight,idVertex,idEdge,i,j;
	...
	for(l=1; l<=t; l++){
		...
		for(idEdge=1; idEdge<=G.nEdges; idEdge++){
			...
			scanf("%d", &weight);
			//.................................................
			for(idVertex=1; idVertex<TotalVertex; idVertex++){
				cond= true;
				for(i=0; Nodos[idVertex][i]!='\0' && city1[i]!='\0'; i++){
					if(Nodos[idVertex][i] != city1[i]){
						cond= false;
						break;
					}
				}
				if(Nodos[idVertex][i]=='\0' && city1[i]=='\0' && cond==true)
				break;
			}
			if(idVertex == TotalVertex){
				for(i=0; city1[i]!='\0'; i++){
					Nodos[idVertex][i]= city1[i];
				}
				Nodos[idVertex][i]= '\0';
				TotalVertex++;
			}
			...
			for(idVertex=1; idVertex<TotalVertex; idVertex++){
				cond= true;
				for(i=0; Nodos[idVertex][i]!='\0' && city2[i]!='\0'; i++){
					if(Nodos[idVertex][i] != city2[i]){
						cond= false;
						break;
					}
				}
				if(Nodos[idVertex][i]=='\0' && city2[i]=='\0' && cond==true)
				break;
			}
			if(idVertex == TotalVertex){
				for(i=0; city2[i]!='\0'; i++){
					Nodos[idVertex][i]= city2[i];
				}
				Nodos[idVertex][i]= '\0';
				TotalVertex++;
			}
		...
		suma=0;
		for(idVertex=1; idVertex<=G.nVertex; idVertex++){
			if(D[idVertex]!=INFINITE)
			suma+= D[idVertex];
			else
			break;
		}
		printf("Case %d: ", l);
		if(idVertex == G.nVertex+1)
		printf("%d\n", suma);
		else
		printf("Impossible\n");
	}
